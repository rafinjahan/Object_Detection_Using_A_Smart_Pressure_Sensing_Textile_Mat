...
df_sens = pd.read_csv(os.path.join(data_folder, 'sensitivity_range32.csv'), sep=';')
...
df_resp = pd.read_csv(os.path.join(data_folder, 'response_time_400g.csv'), sep=';')
df_resp['time_ms'] = (df_resp['timestamp'] - df_resp['timestamp'].iloc[0]) * 1000

df_rec = pd.read_csv(os.path.join(data_folder, 'recovery_time_400g.csv'), sep=';')
...
plt.savefig(os.path.join(images_folder, 'sensitivity_plot.png'), dpi=300)
...
plt.ylabel('Resistance (Ohms)', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.savefig(os.path.join(images_folder, 'response_plot.png'), dpi=300)
...
plt.ylabel('Resistance (Ohms)', fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
plt.savefig(os.path.join(images_folder, 'recovery_plot.png'), dpi=300)
